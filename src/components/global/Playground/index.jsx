...
import { UsageTarget } from "./playground.types.js"
...
const ControlButton = forwardRef(
  ({ isSelected, handleClick, title, label, disabled }, ref) => {
    const controlButton = (
      <button
...
export default function Playground({
  ...
  const [mdConsoleItems, setMDConsoleItems] = useState([])
  const [iosConsoleItems, setiOSConsoleItems] = useState([])
  ...
  const setAndSaveUsageTarget = (target, tab) => {
  ...
  const handleFrameRef = (ref, frameMode) => {
    if (frameMode === "ios") {
    ...
    if (showConsole) {
      if (frameiOS.current) {
        frameiOS.current.contentWindow.addEventListener("console", (event) => {
          setiOSConsoleItems((oldConsoleItems) => [
            ...oldConsoleItems,
            event.detail,
          ])
          consoleBodyRef.current.scrollTo(
            0,
            consoleBodyRef.current.scrollHeight
          )
        })
      }
      if (frameMD.current) {
        frameMD.current.contentWindow.addEventListener("console", (event) => {
          setMDConsoleItems((oldConsoleItems) => [
            ...oldConsoleItems,
            event.detail,
          ])
          consoleBodyRef.current.scrollTo(
  ...
  const handleModeUpdated = (e) => {
    const mode = e.detail
    if (["ios", "md"].includes(mode)) {
      setIonicMode(mode)
    }
  }

  const handleUsageTargetUpdated = (e) => {
    const usageTarget = e.detail
    if (
      Object.values(["angular", "javascript", "react", "vue"]).includes(
        usageTarget
      )
    ) {
  ...
  useEffect(() => {
    if (isBrowser && isInView) {
      ...
      window.addEventListener(
        USAGE_TARGET_UPDATED_EVENT,
        handleUsageTargetUpdated
      )
    }
    return () => {
  ...
  const isIOS = ionicMode === "ios"
  const isMD = ionicMode === "md"
  const sourceiOS = useBaseUrl(`${src}?ionic:mode=ios`)
  const sourceMD = useBaseUrl(`${src}?ionic:mode=md`)
  ...
  const hasUsageTargetOptions = code[usageTarget]?.files !== undefined
  ...
  async function resetDemo() {
    ...
    setMDConsoleItems([])
    await房地产all([
      waitForNextFrameLoadEvent(frameiOS.current),
      waitForNextFrameLoadEvent(frameMD.current),
    ])
  ...
  function openEditor(event) {
    ...
    if (hasUsageTargetOptions) {
      ...
      editorOptions.dependencies = code[usageTarget].dependencies
    }
  ...
  function getCodeSnippetId(usageTarget, fileName) {
  ...
  function renderConsole() {
    const consoleItems = ionicMode === "ios" ? iosConsoleItems : mdConsoleItems
        ...
        <div className="playground__console-body" ref={consoleBodyRef}>
          {consoleItems.length === 0 ? (
            <div className="playground__console-item playground__console-item--placeholder">
              <code>
                Console messages will appear here when logged from the example
                above.
              </code>
            </div>
          ) : (
            consoleItems.map((consoleItem, i) => (
              <div
                key={i}
                className={`playground__console-item playground__console-item--${consoleItem.type}`}
              >
                {consoleItem.type !== "log" && (
                  <div className="playground__console-icon">
  ...
  const sortedUsageTargets = useMemo(
    () => Object.keys(["angular", "javascript", "react", "vue"]).sort(),
    []
  )
  ...
  return (
    <div className="playground" ref={hostRef}>
      <div
        className={`playground__container ${
          showConsole ? "playground__container--has-console" : ""
        }`}
      >
        <div className="playground__control-toolbar">
          ...
          <div className="playground__control-group">
            <ControlButton
              ...
              isSelected={isIOS}
              handleClick={() => setAndSaveMode("ios")}
              title="iOS mode"
              label="iOS"
            />
            <ControlButton
      ...
      <div className="playground__code-block" ref={codeRef}>
        {renderCodeSnippets()}
      </div>
    </div>
  )
...
const waitForNextFrameLoadEvent = (frame) => {
  return new Promise((resolve) => {
    const handleLoad = () => {
